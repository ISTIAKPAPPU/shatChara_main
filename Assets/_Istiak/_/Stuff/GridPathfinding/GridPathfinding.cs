...
namespace _.Stuff.GridPathfinding {
    ...
    public class GridPathfinding {
        ...
        public GridPathfinding(Vector3 worldLowerLeft, Vector3 worldUpperRight, float nodeSize) {
            ...
            float worldHeight = worldUpperRight.y - worldLowerLeft.y-20;
        ...
        public void Initialize(int mapWidth, int mapHeight) {
            ...
            if (showDebug)
            {
                ...
                for (int x = 0; x < mapNodes.Length; x++)
                {
                    for (int y = 0; y < mapNodes[x].Length; y++)
                    {
                        debugTextArray[x, y] = UtilsClass.CreateWorldText(mapNodes[x][y]?.ToString(), null,
                            GetWorldPosition(x, y) + new Vector3(nodeSize, nodeSize) * .5f, 30, Color.white,
        ...
        {
            return new Vector3(x, y) * nodeSize + worldOrigin;
        }
